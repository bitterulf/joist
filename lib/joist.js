...
var Joist = function (DataManager, dep, data) {
  dep.joist = this;
  this.data = data;
  this.dep = dep;
  this.actions = [];
...
Joist.prototype.patch = function (set, unset) {
  ...
  this.reactionUpdate(old, this.dep.dataManager.patch(set, unset));
};
...
Joist.prototype.actionUpdate = function (signal) {
  ...
  _.each(this.actions, function (action) {
    if (action.check(signal, executed)) {
      action.command(dep, signal, executed, function (set, unset) {
        dep.joist.patch(set, unset);
      });
